				 ...
				 float heightScale)
{
	...
	{
		::MessageBox(0, TEXT("readRawFile - FAILED"), 0, 0);
		::PostQuitMessage(0);
	}
	...
	if( !computeVertices() )
	{
		::MessageBox(0, TEXT("computeVertices - FAILED"), 0, 0);
		::PostQuitMessage(0);
	}
	...
	if( !computeIndices() )
	{
		::MessageBox(0, TEXT("computeIndices - FAILED"), 0, 0);
		::PostQuitMessage(0);
	}
}
...
bool Terrain::genTexture(D3DXVECTOR3* directionToLight)
{
	...
	{
		::MessageBox(0, TEXT("lightTerrain() - FAILED"), 0, 0);
		return false;
	}
	...
	if(FAILED(hr))
	{
		::MessageBox(0, TEXT("D3DXFilterTexture() - FAILED"), 0, 0);
		return false;
	}
